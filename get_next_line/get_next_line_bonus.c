...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/20 16:27:03 by vaztar            #+#    #+#             */
/*   Updated: 2020/11/20 17:37:37 by vaztar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	func(char **line, char **arr, int fd, int len)
{
	...
	{
		free(*line);
		if (!(*line = ft_substr(tmp, 0, (int)(tmp - arr[fd]))))
			return (-1);
		if (!(t = ft_strdup(tmp + 1, 0)))
			return (free_vars(arr, line, fd, -1));
		free(arr[fd]);
		arr[fd] = t;
		return (1);
	}
	if (!(t = ft_strdup(*line, 1)))
		return (free_vars(arr, line, fd, -1));
	if (!(*line = ft_strjoin(t, arr[fd])))
		return (free_vars(arr, line, fd, -1));
...
int	read_lines(char **line, char **arr, int *len, int fd)
{
	...
	while (((*len = read(fd, buf, BUFFER_SIZE)) > 0))
	{
		...
		if ((t = ft_strchr(arr[fd], '\n')))
		{
			*t = '\0';
			free(*line);
			if (!(*line = ft_strdup(arr[fd], 0)))
				return (free_vars(arr, line, fd, -1));
			if (!(tt = ft_strdup(t + 1, 0)))
				return (free_vars(arr, line, fd, -1));
			free(arr[fd]);
			arr[fd] = tt;
			return (1);
...
int	get_next_line(int fd, char **line)
{
	...
	{
		free(*line);
		if (!(*line = ft_substr(arr[fd], 0, (int)(tmp - arr[fd]))))
			return (-1);
		if (!(t = ft_strdup(tmp + 1, 0)))
			return (free_vars(arr, line, fd, -1));
		free(arr[fd]);
